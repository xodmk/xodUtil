...
def xodPlot1D(fnum, sig, xLin, pltTitle, pltXlabel, pltYlabel, fsizeX=9, fsizeY=7,
    ...
    """ 1D Matplotlib plot
        ...
        __example:
            # plot 1D waveform wavSrc
            fnum = 1
            pltTitle = 'Input Waveform: ' + wavSrcName
            pltXlabel = 'time-domain wav'
            pltYlabel = 'Magnitude'
            # define a linear space from 0 to 1/2 Fs for x-axis:
            xaxis = np.linspace(0, len(wavSrc), len(wavSrc))
            xodplt.xodPlot1D(fnum, wavSrc, xaxis, pltTitle, pltXlabel, pltYlabel)
            """
...
def xodMultiPlot1D(fnum, sigArray, xLin, pltTitle, pltXlabel, pltYlabel, fsizeX=9, fsizeY=7,
                   colorMap='gnuplot', lnstyle='-', lnwidth=1.00, pltGrid=False, pltBgColor='white'):
...
def xodBarPlot3D(fnum, sigArray, pltTitle, pltXlabel, pltYlabel, pltZlabel,
                 colorMap='gnuplot', pltGrid=False, pltBgColor='black'):
